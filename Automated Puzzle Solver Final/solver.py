...
class DfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        solution_list = [puzzle]
        puzzle_extension = puzzle.extensions()
        if seen is None:
            seen = set()

        seen.add(str(puzzle))
        for puzzle1 in puzzle_extension:
            if str(puzzle1) not in seen:
                solution = self.solve(puzzle1, seen)
                if solution:
...
class BfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        queue_ = Queue()
        path_queue = Queue()
        for puzzle_ in puzzle.extensions():
            queue_.enqueue(puzzle_)
            path_queue.enqueue([puzzle, puzzle_])
        while not queue_.is_empty():
            parent = queue_.dequeue()
            path_ = path_queue.dequeue()
            if parent.is_solved():
                return path_

            else:  # if seen is not None and str(parent) not in seen:
                if str(parent) not in seen and not parent.fail_fast():
                    seen.add(str(parent))
                    # if not parent.fail_fast():
                    parent_extension = parent.extensions()
                    for puzzle_2 in parent_extension:
                        if puzzle_2.is_solved():
                            return path_ + [puzzle_2]
                        else:
                            queue_.enqueue(puzzle_2)
                            path_queue.enqueue(path_ + [puzzle_2])
        return []
