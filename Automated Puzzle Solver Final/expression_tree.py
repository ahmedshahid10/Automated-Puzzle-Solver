...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self._root is None:
            ...
            if self._root in lookup:
                return lookup[self._root]
            else:
                return self._root
        child_expr = []
        for subtree in self._subtrees:
            child_expr.append(subtree.eval(lookup))
        retvalue = child_expr[0]
        for i in range(1, len(child_expr)):
            if self._root == OP_MULTIPLY:
                retvalue = retvalue * child_expr[i]
            else:
                retvalue = retvalue + child_expr[i]
        return retvalue
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            return '()'
        elif isinstance(self._root, int) or self._root.isalpha():
            return f'{self._root}'
        else:
            string = ''
            for i in range(len(self._subtrees)):
                string += self._subtrees[i].__str__()
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if self._root not in OPERATORS and isinstance(self._root, str):
            lookup[self._root] = 0
        elif self._root not in OPERATORS:
            return
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    Q = Queue()
    for subtree in range(1, len(values)):
        if not Q.is_empty():
            t = Q.dequeue()
        else:
            t = root
        for s in values[subtree]:
            if s in OPERATORS:
                new = ExprTree(s, [])
                Q.enqueue(new)
                t.append(new)
