...
class ExpressionTreePuzzle(Puzzle):
    ...
    def extensions(self) -> List[ExpressionTreePuzzle]:
        ...
        for v in self.variables:
            if self.variables[v] == 0:
                for i in range(1, 10):
                    n_puzzle = ExpressionTreePuzzle(
                        self._tree.copy(), self.target)
                    n_puzzle.variables = self.variables.copy()
                    n_puzzle.variables[v] = i
                    final.append(n_puzzle)
        return final
    ...
    def fail_fast(self) -> bool:
        ...
        """
        exten = self.extensions()
        while exten != []:
            current = []
            for e in exten:
                if e._tree.eval(e.variables) == e.target:
                    return False
                current.extend(e.extensions())
            exten = current
        return True
