...
class WordLadderPuzzle(Puzzle):
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        n_words = []
        puzzles = []
        if self.to_word not in self.word_set or len(self.from_word) != \
                len(self.to_word):
            return []
        if self.is_solved():
            return []
        for i in range(0, len(self.from_word)):
            for j in range(0, len(self._chars)):
                n_word = self.from_word[0:i] + self._chars[j] + \
                    self.from_word[i + 1:]
                if n_word in self.word_set and n_word != self.from_word:
                    n_words.append(n_word)
        for item in n_words:
            puzzles.append(WordLadderPuzzle(item, self.to_word, self.word_set))
        return puzzles
    ...
    def get_difficulty(self) -> str:
        ...
        if len(solution) == 1 or len(solution) == 2:
            return TRIVIAL
        if len(solution) == 3:
            return EASY
        if 3 < len(solution) < 5:
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={'allowed-io': ['load_words'],
